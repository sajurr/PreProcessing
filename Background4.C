...
// Open files
	//TFile *file = TFile::Open("/mnt/storage/lborgna/BkgMatched/Final/BkgAll6_HighPt_Train.root");	// switch file between signal and background here
	TFile *file = TFile::Open("/mnt/storage/lborgna/BkgMatched/Final/Bkg_Test_m600.root");
...
// Define histogram list and output file
  ...
  TFile* outfile = new TFile("/mnt/storage/lborgna/DijetPreP/Pre_Bkg7_m600_Test_25p.root", "RECREATE");
...
// Initialize
    ...
    //vector <Float_t> *clus_eta = 0;
    //vector <Float_t> *clus_phi = 0;
    //vector <Float_t> *clus_energy   = 0;
...
// Fill histo1 with clusters
	...
	for (int i = 0; i<tree_size; i++) {
		...
		//while ((fjet_pt > 1500 * GeV && fjet_pt < 2000 * GeV) && (fjet_truthJet_pt > 1500 * GeV && fjet_truthJet_pt < 2000 * GeV))  {
	//	while ((fjet_fatjet_dRmatched_particle_flavor == 24) || (fjet_fatjet_dRmatched_particle_flavor == -24)) {	//PGID, comment out for background file
			particlehits++;
			...
			for (int q = 0; q<clus_eta->size(); q++) {					
				...
				//while (clus_energy->at(q) != max) {
					...
					clus_E	 	= clus_energy->at(q);
			
					clus_theta = 2*atan(exp(-clus_N));
					clus_et = abs(clus_E*cos(clus_theta)); //	transverse energy, var 5 (use in report)
					...
					//clus_Nfin = (clus_N*cos(theta) - clus_P*sin(theta))*sf;
					//clus_Pfin = (clus_N*sin(theta) + clus_P*cos(theta))*sf;
                    clus_Nfin = clus_N;
                    clus_Pfin = clus_P;
